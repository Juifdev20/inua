...
public class PatientDocumentService {
    ...
    public PatientDocumentDTO generatePatientDossier(Long consultationId) {
        ...
        try {
            // Récupérer la consultation avec toutes ses relations (évite LazyInitializationException)
            Consultation consultation = consultationRepository.findByIdWithExams(consultationId)
                    .orElseThrow(() -> new RuntimeException("Consultation non trouvée: " + consultationId));
            
            // S'assurer que le patient est chargé
            if (consultation.getPatient() == null) {
                throw new RuntimeException("Patient non associé à la consultation: " + consultationId);
            }
            ...
            if (patient == null) {
                throw new RuntimeException("Patient non trouvé pour la consultation: " + consultationId);
            }
            String patientName = (patient.getFirstName() != null ? patient.getFirstName() : "") + " " + 
                                 (patient.getLastName() != null ? patient.getLastName() : "");
            String dateStr = LocalDateTime.now().format(DATE_FORMATTER);

            // Générer le nom du fichier unique (nettoyé)
            ...
            // Créer le répertoire parent si nécessaire (important!)
            File parentDir = pdfFile.getParentFile();
            if (parentDir != null && !parentDir.exists()) {
                log.info("📁 [PATIENT_DOCUMENT] Création du répertoire: {}", parentDir.getAbsolutePath());
                parentDir.mkdirs();
            }

            // Générer le PDF
            byte[] pdfContent = generatePDFToBytes(consultation, patient);
            
            // Sauvegarder sur disque
            log.info("💾 [PATIENT_DOCUMENT] Sauvegarde du fichier: {}", pdfFile.getAbsolutePath());
            ...
            PatientDocument document = PatientDocument.builder()
                    ...
                    .content(pdfContent)  // ✅ Stockage en base pour accès patient
    ...
    /**
     * Vérifie si un dossier existe déjà pour une consultation
     * @param consultationId ID de la consultation
     * @return true si un dossier existe déjà
     */
    public boolean dossierExistsForConsultation(Long consultationId) {
        return patientDocumentRepository.existsByConsultationId(consultationId);
    }

    /**
     * Génère un nom de fichier unique (sanitisé pour Windows/Linux)
     */
    private String generateUniqueFileName(Consultation consultation, Patient patient, String dateStr) {
        String firstName = patient.getFirstName();
        String lastName = patient.getLastName();
        
        // Nettoyer les noms (espaces -> underscores, caractères spéciaux -> vide)
        String patientFirstName = sanitizeFileName(firstName != null ? firstName : "Inconnu");
        String patientLastName = sanitizeFileName(lastName != null ? lastName : "Patient");
        
        // Nettoyer aussi le code consultation
        String rawCode = consultation.getConsultationCode() != null 
                ? consultation.getConsultationCode() 
                : "C" + consultation.getId().toString();
        String consultationCode = sanitizeFileName(rawCode);
    ...
    private String sanitizeFileName(String input) {
        ...
        return input.replaceAll("[\\s\\\\/:*?\"<>|]", "_").replaceAll("_+", "_").trim();
    }
    ...
    /**
     * Génère le PDF du dossier patient et retourne les bytes
     */
    private byte[] generatePDFToBytes(Consultation consultation, Patient patient) throws Exception {
        log.info("📄 [PATIENT_DOCUMENT] Génération du PDF en mémoire");
        ...
        try {
            ...
            log.info("📄 [PATIENT_DOCUMENT] Document PDF créé, génération du contenu...");
            generatePDFContent(document, consultation, patient);
            
            byte[] content = baos.toByteArray();
            log.info("✅ [PATIENT_DOCUMENT] PDF généré: {} bytes", content.length);
            return content;
        } catch (Exception e) {
            log.error("❌ [PATIENT_DOCUMENT] Erreur génération PDF bytes: {}", e.getMessage(), e);
            throw e;
        }
    }

    /**
     * Génère le PDF du dossier patient sur disque (méthode legacy)
     */
    ...
    /**
     * Génère le contenu du PDF (partagé entre les deux méthodes)
     */
    private void generatePDFContent(com.lowagie.text.Document document, Consultation consultation, Patient patient) throws Exception {
        ...
        String patientInfo = String.format("Patient: %s %s | Code: %s | Date: %s",
                ...
                patient.getLastName() != null ? patient.getLastName() : "",
                patient.getPatientCode() != null ? patient.getPatientCode() : "N/A",
