...
public class LabController {
    ...
    /**
     * ★ STATISTIQUES DU LABORATOIRE - Données réelles pour le dashboard
     */
    @GetMapping("/stats")
    @PreAuthorize("hasAnyRole('LABORATOIRE', 'ADMIN', 'LAB_TECH')")
    @Operation(summary = "Statistiques du laboratoire")
    public ResponseEntity<Map<String, Object>> getLabStats() {
        ...
        try {
            ...
            long pendingExams = allExams.stream()
                ...
                .count();
            long resultsEntered = allExams.stream()
                .filter(e -> e.getResultValue() != null && !e.getResultValue().isEmpty())
                .filter(e -> e.getStatus() == PrescribedExamStatus.COMPLETED || 
                            e.getStatus() == PrescribedExamStatus.RESULTS_AVAILABLE)
                .count();
            long validatedExams = allExams.stream()
                .filter(e -> e.getStatus() == PrescribedExamStatus.RESULTS_AVAILABLE ||
                            e.getStatus() == PrescribedExamStatus.DELIVERED_TO_DOCTOR)
                .count();

            log.info("🔬 [LAB CTRL] Stats - Total: {}, En attente: {}, Résultats saisis: {}, Validés: {}",
                totalExams, pendingExams, resultsEntered, validatedExams);
            ...
            log.error("❌ [LAB CTRL] Erreur stats: {}", e.getMessage(), e);
            return ResponseEntity.internalServerError().body(Map.of(
                "success", false,
                "message", "Erreur: " + e.getMessage()
            ));
    ...
    /**
     * ★ HISTORIQUE DES EXAMENS - Récupérer tous les examens avec résultats
     */
    @GetMapping("/history")
    @PreAuthorize("hasAnyRole('LABORATOIRE', 'ADMIN', 'LAB_TECH')")
    @Operation(summary = "Historique des examens avec résultats")
    public ResponseEntity<Map<String, Object>> getLabHistory() {
        ...
        try {
            ...
            List<Map<String, Object>> historyItems = allExams.stream()
                ...
                .map(e -> {
                    ...
                    item.put("id", e.getId());
                    item.put("patient", c != null && c.getPatient() != null ? 
                        c.getPatient().getFirstName() + " " + c.getPatient().getLastName() : "Inconnu");
                    item.put("patientId", c != null && c.getPatient() != null ? c.getPatient().getId() : null);
                    item.put("patientCode", c != null && c.getPatient() != null ? c.getPatient().getPatientCode() : null);
                    item.put("date", e.getResultEnteredAt() != null ? 
                        e.getResultEnteredAt().toLocalDate().toString() : 
                        (e.getCreatedAt() != null ? e.getCreatedAt().toLocalDate().toString() : ""));
                    item.put("exam", e.getServiceName());
                    item.put("result", e.getResultValue() + (e.getUnit() != null ? " " + e.getUnit() : ""));
                    item.put("resultValue", e.getResultValue());
                    item.put("unit", e.getUnit());
                    item.put("isCritical", e.getIsCritical());
                    item.put("labComment", e.getLabComment());
                    item.put("status", e.getStatus().name());
                    item.put("technician", e.getResultEnteredBy());
                    return item;
                })
                .sorted((a, b) -> ((String) b.get("date")).compareTo((String) a.get("date"))) // Plus récents d'abord
                .collect(Collectors.toList());

            log.info("🔬 [LAB CTRL] {} examens dans l'historique", historyItems.size());
            ...
            log.error("❌ [LAB CTRL] Erreur historique: {}", e.getMessage(), e);
            return ResponseEntity.internalServerError().body(Map.of(
                "success", false,
                "message", "Erreur: " + e.getMessage()
            ));
    ...
    /**
     * ★ ALERTES LABORATOIRE - Récupère les alertes intelligentes pour l'utilisateur
     */
    @GetMapping("/alerts")
    @PreAuthorize("hasAnyRole('LABORATOIRE', 'ADMIN', 'LAB_TECH')")
    @Operation(summary = "Alertes intelligentes du laboratoire")
    public ResponseEntity<Map<String, Object>> getLabAlerts(
            @RequestParam(required = false) Boolean generate) {
        
        log.info("🔬 [LAB CTRL] Récupération des alertes - Generate: {}", generate);

        try {
            // Optionnellement générer de nouvelles alertes
            if (Boolean.TRUE.equals(generate)) {
                labAlertService.generateLabAlerts();
            }
            ...
            List<Map<String, Object>> alerts = labAlertService.getLabAlertsForUser(user.getId());
            ...
            log.error("❌ [LAB CTRL] Erreur alertes: {}", e.getMessage(), e);
            return ResponseEntity.internalServerError().body(Map.of(
                "success", false,
                "message", "Erreur: " + e.getMessage()
            ));
    ...
    private LabQueueItemDTO mapToLabQueueItem(Consultation c) {
        List<LabQueueItemDTO.ExamItemDTO> exams = c.getPrescribedExams().stream()
            .map(exam -> LabQueueItemDTO.ExamItemDTO.builder()
                ...
                .unit(exam.getUnit())
                .referenceMin(exam.getReferenceMin())
                .referenceMax(exam.getReferenceMax())
