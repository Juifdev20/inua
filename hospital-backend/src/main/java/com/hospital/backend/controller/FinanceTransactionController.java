...
public class FinanceTransactionController {
    ...
    @Operation(summary = "Valider une dépense avec scan de facture",
            description = "Upload du scan optionnel si pièce justificative fournie. Mode: IMMEDIAT (décaissement) ou CREDIT (dette)")
    public ResponseEntity<?> validerDepense(
            @PathVariable Long id,
            @Parameter(description = "Scan de la facture fournisseur (PDF/JPG/PNG) - optionnel si pièce justificative existe", required = false)
            @RequestParam(value = "scanFacture", required = false) MultipartFile scanFacture,
            @Parameter(description = "Mode paiement: IMMEDIAT ou CREDIT")
            @RequestParam(value = "modePaiement", required = false) String modePaiementStr,
        ...
        try {
            ...
            // Parser le mode de paiement (robustesse pour multipart)
            PaiementMode modePaiement = null;
            if (modePaiementStr != null && !modePaiementStr.isBlank()) {
                try {
                    modePaiement = PaiementMode.valueOf(modePaiementStr.trim().toUpperCase());
                } catch (IllegalArgumentException e) {
                    log.warn("Mode paiement invalide '{}', fallback sur caisseId", modePaiementStr);
                }
            }
            // Si caisseId fourni mais mode null/invalide → forcer IMMEDIAT
            if (modePaiement == null && caisseId != null) {
                modePaiement = PaiementMode.IMMEDIAT;
                log.info("CaisseId fourni ({}), mode forcé à IMMEDIAT", caisseId);
            }

            log.info("📤 Validation dépense ID: {}, mode: {}, caisseId: {}, fichier: {}, par: {}", id, modePaiement,
                    caisseId,
