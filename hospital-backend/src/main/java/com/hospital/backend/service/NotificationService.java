...
public class NotificationService {
    ...
    public void createAndSend(User recipient, String title, String message, NotificationType type, Long referenceId) {
        ...
        // Topic spécifique à l'utilisateur (pour notifications privées)
        String userDestination = "/topic/notifications/" + recipient.getId();
        messagingTemplate.convertAndSend(userDestination, savedNotification);
        
        // Topic général pour toutes les notifications (pour les admins connectés)
        String generalDestination = "/topic/notifications";
        messagingTemplate.convertAndSend(generalDestination, Map.of(
            "id", savedNotification.getId(),
            "content", message,
            "title", title,
            "type", type.name(),
            "timestamp", savedNotification.getCreatedAt() != null ? savedNotification.getCreatedAt().toString() : java.time.LocalDateTime.now().toString(),
            "userId", recipient.getId()
        ));
        
        log.info("📢 Notification envoyée à l'utilisateur {} et au topic général: {} - {}", recipient.getId(), title, message);
    }
    ...
    public void notifyPaymentConfirmed(com.hospital.backend.entity.PharmacyOrder order) {
        ...
        // Envoyer via WebSocket au topic Pharmacie
        String destination = "/topic/pharmacy/paiements-confirme";
        messagingTemplate.convertAndSend(destination, Map.of(
            "orderId", order.getId(),
