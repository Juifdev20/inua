...
export const admissionService = {
   ...
   * Seules les admissions en statut EN_ATTENTE peuvent être annulées
   */
  cancelAdmission: async (admissionId, reason = null) => {
    try {
      console.log(`📡 Appel API: POST /v1/reception/admissions/${admissionId}/cancel`);
      const payload = reason ? { reason } : {};
      const response = await api.post(`/v1/reception/admissions/${admissionId}/cancel`, payload);
      return response.data;
    } catch (error) {
      console.error('❌ Erreur cancelAdmission:', error.message);
      throw error.response?.data || { message: error.message };
    }
  }
};
