...
const Consultations = () => {
  ...
  const fetchConsultations = async () => {
    try {
      ...
      console.log('📋 CONSULTATIONS CHARGÉES:', response.data);
    } catch (error) {
  ...
  // ✅ FONCTION CORRIGÉE : Utilise le nouvel endpoint /terminer
  const terminerConsultation = async () => {
    ...
    try {
      ...
      console.log('📡 Envoi données vers:', `/api/v1/doctor/consultations/${selectedConsultation.id}/terminer`);
  ...
  const handleUpdateConsultation = async (id, data) => {
    try {
      const token = localStorage.getItem('token');
      
      // ✅ URL CORRIGÉE : Utiliser le nouvel endpoint
      const requestData = {
        diagnostic: data.diagnostic,
        examensIds: data.examens || []
      };

      console.log('📡 Envoi données vers:', `/api/v1/doctor/consultations/${id}/terminer`);
      console.log('📋 Payload:', requestData);

      const response = await axios.put(
        `/api/v1/doctor/consultations/${id}/terminer`,
        requestData,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
          ...
          {consultations.map(consultation => (
            <Card
              ...
              extra={
                ...
                >
                  {consultation.status === 'ATTENTE_PAIEMENT_LABO' ? 'En attente de paiement' : 'Terminer & Envoyer'}
                </Button>
              }
              style={{ marginBottom: '16px' }}
            >
              <p><strong>Date:</strong> {new Date(consultation.consultationDate).toLocaleDateString()}</p>
