...
public class DoctorController {
    ...
    @Autowired
    private LabAlertService labAlertService;
        ...
        try {
            ...
            if (hasExams) {
                ...
                for (Map<String, Object> examData : exams) {
                    String examName = createAndSaveExamAndReturnName(consultation, examData);
                    if (examName != null) examNames.add(examName);
                }
                ...
                try {
                    ...
                    labAlertService.notifyExamsPrescribed(doctorName, patientName, examNames, id);
                } catch (Exception e) {
                    log.error("❌ Erreur notification labo: {}", e.getMessage());
                }
    ...
    // ══════════════════════════════════════════════════════════════════
    // MÉTHODE UTILITAIRE : création des PrescribedExam avec VRAI EXAMEN
    // Utilise la nouvelle entité Examen avec prix, unité, valeurs de référence
    // ══════════════════════════════════════════════════════════════════

    private String createAndSaveExamAndReturnName(Consultation consultation, Map<String, Object> examData) {
        try {
            // ★ NOUVEAU: Utilise examenId au lieu de serviceId
            Long examenId = Long.valueOf(examData.get("examenId").toString());
            String note = (String) examData.getOrDefault("note", "");

            log.info("🧪 Création examen pour examenId={} note='{}'", examenId, note);

            // ★ Récupérer le vrai examen de labo
            ...
            exam.setExamen(examen); // ★ Lien vers le vrai examen
            exam.setServiceName(examen.getNom()); // Nom de l'examen
            exam.setUnitPrice(examen.getPrix()); // ★ Prix réel de l'examen
            exam.setQuantity(1);
            exam.setTotalPrice(examen.getPrix()); // ★ Total = prix unitaire
            exam.setDoctorNote(note);
            exam.setActive(true);
            exam.setStatus(PrescribedExamStatus.PRESCRIBED);
            exam.setCreatedAt(LocalDateTime.now());

            // ★ Stockage des valeurs de référence pour affichage au labo
            exam.setUnit(examen.getUnite());
            exam.setReferenceMin(examen.getValeurMinReference() != null ? examen.getValeurMinReference().toString() : null);
            exam.setReferenceMax(examen.getValeurMaxReference() != null ? examen.getValeurMaxReference().toString() : null);

            prescribedExamRepository.save(exam);
            log.info("✅ Examen prescrit sauvegardé (ID={}) - {} ({} CDF) pour consultation {}",
                    exam.getId(), examen.getNom(), examen.getPrix(), consultation.getId());
