...
public class PharmacyServiceImpl implements PharmacyService {
    ...
    public PharmacyOrderDTO createOrder(PharmacyOrderDTO orderDTO) {
        ...
        // ✅ DÉCRÉMENTER LE STOCK seulement si:
        // 1. Vente directe ET statut PAYEE (paiement immédiat)
        // 2. PAS EN_ATTENTE_PAIEMENT (attente validation finance)
        boolean shouldDecrementStock = 
            (savedOrder.getOrderType() == PharmacyOrderType.VENTE_DIRECTE && 
             savedOrder.getStatus() == PharmacyOrderStatus.PAYEE) ||
            (savedOrder.getOrderType() == PharmacyOrderType.ORDONNANCE_EXTERNE &&
             savedOrder.getStatus() == PharmacyOrderStatus.PAYEE);
        
        if (shouldDecrementStock) {
            log.info("🔄 [STOCK] Décrémentation du stock pour la commande: {}", savedOrder.getOrderCode());
            decrementStockForOrder(savedOrder);
        } else if (savedOrder.getStatus() == PharmacyOrderStatus.EN_ATTENTE_PAIEMENT) {
            log.info("⏳ [STOCK] Stock NON décrémenté - En attente validation finance: {}", savedOrder.getOrderCode());
            // TODO: Optionnel - Réserver le stock ici (soft reservation)
        }
    ...
    /**
     * Décrémente le stock pour une commande - appelé lors de la confirmation paiement
     */
    @Transactional
    public void decrementStockForOrder(PharmacyOrder order) {
        for (PharmacyOrderItem item : order.getItems()) {
    ...
    public PharmacyOrderDTO processPayment(Long orderId, BigDecimal amountPaid, String paymentMethod, Boolean allowPartialPayment) {
        ...
        // Convertir le paymentMethod string en enum avec gestion d'erreur
    ...
    /**
     * 💰 CONFIRMATION DE PAIEMENT PAR LA FINANCE
     * Cette méthode est appelée par le module Finance pour confirmer un paiement
     * Déclenche le déstockage et met à jour le statut
     */
    @Override
    @Transactional
    public PharmacyOrderDTO confirmPaymentByFinance(Long orderId, Long cashierId, BigDecimal amountPaid, String paymentMethod) {
        log.info("💰 [FINANCE] Confirmation paiement commande {} par caissier {}", orderId, cashierId);
        ...
        if (order.getStatus() != PharmacyOrderStatus.EN_ATTENTE_PAIEMENT) {
            throw new RuntimeException("La commande n'est pas en attente de paiement. Statut actuel: " + order.getStatus());
        }
        
        // Mettre à jour les informations de paiement
        order.setAmountPaid(amountPaid);
        order.setTotalAmount(amountPaid); // Le montant payé devient le montant total
        ...
        log.info("🔄 [STOCK] Décrémentation du stock après confirmation finance pour: {}", order.getOrderCode());
        decrementStockForOrder(order);
        
        // Mettre à jour le statut à PAYEE
        order.setStatus(PharmacyOrderStatus.PAYEE);
        
        // Enregistrer qui a validé (caissier)
        User cashier = userRepository.findById(cashierId)
                .orElseThrow(() -> new ResourceNotFoundException("Caissier non trouvé: " + cashierId));
        order.setValidatedBy(cashier);
        order.setValidatedAt(LocalDateTime.now());
        
        PharmacyOrder updatedOrder = pharmacyOrderRepository.save(order);
        
        // 🔔 NOTIFIER LA PHARMACIE que le paiement est confirmé
        try {
        ...
        try {
            recordRevenueInLedger(updatedOrder, cashier);
        } catch (Exception e) {
            log.error("❌ Erreur enregistrement comptabilité: {}", e.getMessage());
        }
    ...
    /**
     * Enregistre la vente dans le Grand Livre (Comptabilité)
     */
    private void recordRevenueInLedger(PharmacyOrder order, User cashier) {
        if (revenueService != null && order.getTotalAmount() != null && order.getTotalAmount().compareTo(BigDecimal.ZERO) > 0) {
            try {
                // Créer le UserDTO pour createdBy
                RevenueDTO.UserDTO userDTO = RevenueDTO.UserDTO.builder()
                        .id(cashier.getId())
                        .firstName(cashier.getFirstName())
                        .lastName(cashier.getLastName())
                        .build();
                
                // Construire le RevenueDTO avec le Builder
                RevenueDTO revenueDTO = RevenueDTO.builder()
                        .amount(order.getTotalAmount())
                        .source(com.hospital.backend.entity.Revenue.RevenueSource.PHARMACIE)
                        .description("Vente médicaments - Commande " + order.getOrderCode() + 
                            " - Client: " + (order.getCustomerName() != null ? order.getCustomerName() : "Client comptoir"))
                        .date(LocalDateTime.now())
                        .paymentMethod(order.getPaymentMethod())
                        .currency(order.getCurrency() != null ? order.getCurrency() : com.hospital.backend.entity.Currency.USD)
                        .createdBy(userDTO)
                        .build();
                
                // Créer le revenu avec l'ID du caissier
                revenueService.createRevenue(revenueDTO, cashier.getId());
                log.info("💰 [COMPTA] Revenu enregistré dans le Grand Livre: {} $", order.getTotalAmount());
            } catch (Exception e) {
                log.error("❌ [COMPTA] Erreur enregistrement revenu: {}", e.getMessage());
            }
        }
    }
    ...
    public PharmacyOrderDTO convertPrescriptionToOrder(Long prescriptionId, Map<String, Object> options) {
        ...
        // Déterminer le statut de la commande basé sur la prescription
        boolean isPrescriptionPaid = prescription.getStatus() == PrescriptionStatus.PAYEE;
        PharmacyOrderStatus orderStatus = isPrescriptionPaid ? PharmacyOrderStatus.PAYEE : PharmacyOrderStatus.EN_PREPARATION;
        ...
        PharmacyOrder order = PharmacyOrder.builder()
            ...
            .amountPaid(isPrescriptionPaid ? null : BigDecimal.ZERO)  // Sera mis à jour après calcul
        ...
        for (PrescriptionItem prescriptionItem : prescription.getItems()) {
            ...
            if (quantity == null || quantity <= 0) {
                quantity = 1; // Valeur par défaut si null ou invalide
            }
        ...
        // Mettre à jour le montant total et le montant payé si prescription déjà payée
        order.setTotalAmount(totalAmount);
        if (isPrescriptionPaid) {
            order.setAmountPaid(totalAmount);
        }
        order = pharmacyOrderRepository.save(order);
        log.info("✅ [PRESCRIPTION] Commande créée: ID={}, Code={}, Statut={}, Total={}, Payé={}", 
            order.getId(), order.getOrderCode(), order.getStatus(), order.getTotalAmount(), order.getAmountPaid());
