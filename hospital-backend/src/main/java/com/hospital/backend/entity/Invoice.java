...
@AllArgsConstructor
@Builder
@EqualsAndHashCode(exclude = {"consultation", "patient", "items", "createdBy"})  // 👈 CORRECTION: Exclut les références circulaires/lazy pour éviter recursion
@ToString(exclude = {"consultation", "patient", "items", "createdBy"})  // 👈 CORRECTION: Évite recursion dans toString()
public class Invoice {
    ...
    // ✅ NOUVEAU: Source du département qui a généré la facture
    @Enumerated(EnumType.STRING)
    @Column(name = "department_source")
    private DepartmentSource departmentSource;

    // ✅ Devise de la facture (CDF ou USD selon les médicaments)
    @Enumerated(EnumType.STRING)
