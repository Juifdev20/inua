...
public class PricingController {
    ...
    public ResponseEntity<Map<String, Object>> calculateAdmissionPrice(
        ...
        try {
            ...
            // 4. Forcer USD pour tous les montants (pas de conversion)
            BigDecimal ficheAmountUSD = ficheAmount; // Déjà en USD depuis getFicheAmount
            
            // 5. Calculer le total en USD
            BigDecimal totalAmount = ficheAmountUSD.add(consulAmount);
            
            // 6. Préparer le message
            String message;
            if (hasActiveFile) {
                message = "Patient ancien - Fiche exonérée (valide 12 mois)";
            } else {
                message = String.format("Nouveau patient - Frais de dossier: %s USD", ficheAmountUSD);
            }
            
            // Construire la réponse (toujours en USD)
            AdmissionPricingDTO pricing = AdmissionPricingDTO.builder()
                .patientId(patientId)
                .serviceId(serviceId)
                .ficheAmount(ficheAmountUSD)  // Toujours en USD
                .consulAmount(consulAmount)
                .totalAmount(totalAmount)
                .ficheRequired(!hasActiveFile)
                .hasActiveFile(hasActiveFile)
                .serviceName(serviceName)
                .ficheUnitPrice(ficheAmountUSD)
     ...
     * Taux de change: 1 USD = 2600 CDF (à ajuster selon le taux du marché)
     */
    private BigDecimal convertCurrency(BigDecimal amount, Currency from, Currency to) {
        ...
        // Taux de change fixe (peut être externalisé en configuration)
        final BigDecimal USD_TO_CDF = new BigDecimal("2600");
        final BigDecimal CDF_TO_USD = new BigDecimal("0.0003846"); // 1/2600
