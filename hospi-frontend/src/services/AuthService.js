 ...
 * @param {string} refreshToken - Refresh token pour renouvellement automatique
 */
export const saveAuthData = (token, user, rememberMe = true, refreshToken = null) => {
  try {
    // Stockage du token
    safeLocalStorageSet(STORAGE_KEYS.TOKEN, token);
    localStorage.setItem(STORAGE_KEYS.TOKEN, token);
    
    // Stockage du refresh token (si fourni)
    if (refreshToken) {
      localStorage.setItem(STORAGE_KEYS.REFRESH_TOKEN, refreshToken);
    }
    
    // Stockage des données utilisateur (si fourni)
    if (user) {
      const userData = {
        id: user.id,
        role: user.role,
    ...
    console.log('[AuthService] ✅ Données de session sauvegardées (refresh token:', !!refreshToken + ')');
    return true;
  } catch (error) {
