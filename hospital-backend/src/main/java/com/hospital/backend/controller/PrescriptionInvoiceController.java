...
public class PrescriptionInvoiceController {
        ...
        try {
            ...
            for (InvoiceDTO invoice : invoices) {
                log.info("📋 [DEBUG] Facture: {} - Patient: {} - Montant: {} {} - Statut: {}", 
                    invoice.getInvoiceCode(), invoice.getPatientName(), invoice.getTotalAmount(), 
                    invoice.getCurrency() != null ? invoice.getCurrency() : "NULL", invoice.getStatus());
            }
    ...
    @GetMapping("/stats")
    @PreAuthorize("hasAnyRole('ADMIN', 'CASHIER', 'PHARMACIST', 'PHARMACIE', 'FINANCE', 'CAISSIER', 'PHARMACY')")
