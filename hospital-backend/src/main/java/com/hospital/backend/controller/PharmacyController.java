...
public class PharmacyController {
    ...
    @GetMapping("/prescriptions/{prescriptionId:[0-9]+}")
    @PreAuthorize("hasAnyAuthority('ROLE_PHARMACIE', 'ROLE_PHARMACY', 'ROLE_ADMIN')")
    ...
    @PostMapping("/prescriptions/{prescriptionId:[0-9]+}/convert")
    @PreAuthorize("hasAnyAuthority('ROLE_PHARMACIE', 'ROLE_PHARMACY', 'ROLE_ADMIN')")
    @Operation(summary = "Convertir une prescription médicale en commande pharmacie")
    public ResponseEntity<?> convertPrescriptionToOrder(
        ...
        try {
            ...
            log.error("Erreur conversion prescription {}: {}", prescriptionId, e.getMessage(), e);
            return ResponseEntity.badRequest().body(Map.of(
                "error", "Erreur conversion",
    ...
    /**
     * 🔧 NOUVEAU: Récupérer les mouvements de stock par statut
     * Utilisé par la pharmacie pour voir les achats en attente de validation
     */
    @GetMapping("/stock/movements/status/{status}")
    @PreAuthorize("hasAnyRole('PHARMACY', 'PHARMACIE', 'ADMIN', 'FINANCE', 'CAISSE')")
    @Operation(summary = "Récupérer les mouvements de stock par statut (EN_ATTENTE_VALIDATION, VALIDE, etc.)")
    public ResponseEntity<?> getStockMovementsByStatus(@PathVariable String status) {
        try {
            log.info("📦 Récupération des mouvements de stock avec statut: {}", status);
            
            // Convertir le string en enum
            StockMovement.MovementStatus movementStatus;
            try {
                movementStatus = StockMovement.MovementStatus.valueOf(status);
            } catch (IllegalArgumentException e) {
                return ResponseEntity.badRequest().body(Map.of(
                    "error", "Statut invalide. Valeurs acceptées: EN_ATTENTE_VALIDATION, VALIDE, ANNULE, REJETE"
                ));
            }
            
            List<StockMovement> movements = stockMovementRepository.findByStatus(movementStatus);
            
            // Enrichir avec les informations du médicament
            List<Map<String, Object>> result = movements.stream().map(m -> {
                Map<String, Object> map = new HashMap<>();
                map.put("id", m.getId());
                map.put("medication", Map.of(
                    "id", m.getMedication().getId(),
                    "name", m.getMedication().getName(),
                    "medicationCode", m.getMedication().getMedicationCode()
                ));
                map.put("quantityChange", m.getQuantityChange());
                map.put("previousStock", m.getPreviousStock());
                map.put("newStock", m.getNewStock());
                ...
                map.put("financeTransactionId", m.getFinanceTransactionId());
                map.put("notes", m.getNotes());
                map.put("createdAt", m.getCreatedAt());
                return map;
            }).collect(Collectors.toList());
            
            log.info("✅ {} mouvements trouvés avec statut {}", movements.size(), status);
            return ResponseEntity.ok(Map.of(
                "status", status,
                "movements", result,
                "count", movements.size()
            ...
            log.error("❌ Erreur récupération mouvements par statut: {}", e.getMessage());
            return ResponseEntity.internalServerError().body(Map.of("error", e.getMessage()));
        }
    }
