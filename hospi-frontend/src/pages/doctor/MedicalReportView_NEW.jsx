...
const MedicalReportView = () => {
  ...
  if (!report) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 p-4">
        ...
        <p className="text-gray-600 text-center">Dossier introuvable</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 print:bg-white">
      {/* Header - Non imprimable - Responsive */}
      <div className="print:hidden bg-white border-b p-3 md:p-4 sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1000px] mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base md:text-lg font-semibold text-gray-900">Fiche Medicale Individuelle</h1>
              <p className="text-xs md:text-sm text-gray-500">Consultation #{report.consultationCode}</p>
            </div>
          </div>
      ...
      <div className="max-w-[900px] mx-auto p-3 md:p-6 print:p-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div>
                ...
                <p className="text-xs md:text-sm text-emerald-100">Systeme de Gestion Hospitaliere</p>
              </div>
            </div>
            <div className="text-left sm:text-right">
              <p className="text-xs md:text-sm font-medium">FICHE MEDICALE INDIVIDUELLE</p>
              <p className="text-[10px] md:text-xs text-emerald-100">Genere: {formatDate(report.reportGeneratedAt)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-2 border-emerald-600 border-t-0 rounded-b-lg bg-white print:border-2 print:rounded-none">
            ...
            <div className="p-3 md:p-4 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              <div className="space-y-2 md:space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                  ...
                  <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.patientName}</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                  <span className="text-xs md:text-sm font-semibold text-gray-700 sm:min-w-[100px]">Code Patient:</span>
                  <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.patientCode}</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                  <span className="text-xs md:text-sm font-semibold text-gray-700 sm:min-w-[100px]">Age:</span>
                  <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.patientAge} ans</span>
                </div>
              </div>
              <div className="space-y-2 md:space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                  <span className="text-xs md:text-sm font-semibold text-gray-700 sm:min-w-[100px]">Sexe:</span>
                  <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.patientGender}</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                  <span className="text-xs md:text-sm font-semibold text-gray-700 sm:min-w-[100px]">Telephone:</span>
                  <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.patientPhone || '-'}</span>
                </div>
                <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                  <span className="text-xs md:text-sm font-semibold text-gray-700 sm:min-w-[100px]">Consultation:</span>
                  <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{formatDate(report.consultationDate)}</span>
                </div>
              </div>
            </div>
          </MobileSection>
          ...
          <MobileSection title="Medecin Traitant" icon={Stethoscope} sectionKey="doctor">
            <div className="p-3 md:p-4 grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                <span className="text-xs md:text-sm font-semibold text-gray-700 sm:min-w-[100px]">Nom:</span>
                <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.doctorName}</span>
              </div>
              <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                <span className="text-xs md:text-sm font-semibold text-gray-700 sm:min-w-[100px]">Specialite:</span>
                <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.doctorSpecialty}</span>
              </div>
            </div>
          </MobileSection>
          ...
          {report.triageInfo && (
            <MobileSection title="Constantes Vitales" icon={Activity} sectionKey="triage">
              <div className="p-3 md:p-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
                  <div className="border border-gray-200 rounded p-2 md:p-3 text-center bg-gray-50">
                    <p className="text-[10px] md:text-xs text-gray-500 mb-1">Temperature</p>
                    <p className="text-base md:text-lg font-semibold text-emerald-700">{report.triageInfo.temperature || '-'}°C</p>
                  </div>
                  <div className="border border-gray-200 rounded p-2 md:p-3 text-center bg-gray-50">
                    <p className="text-[10px] md:text-xs text-gray-500 mb-1">Tension</p>
                    <p className="text-base md:text-lg font-semibold text-emerald-700">{report.triageInfo.tensionArterielle || '-'}</p>
                  </div>
                  <div className="border border-gray-200 rounded p-2 md:p-3 text-center bg-gray-50">
                    <p className="text-[10px] md:text-xs text-gray-500 mb-1">Poids</p>
                    <p className="text-base md:text-lg font-semibold text-emerald-700">{report.triageInfo.poids || '-'} kg</p>
                  </div>
                  <div className="border border-gray-200 rounded p-2 md:p-3 text-center bg-gray-50">
                    <p className="text-[10px] md:text-xs text-gray-500 mb-1">Taille</p>
                    <p className="text-base md:text-lg font-semibold text-emerald-700">{report.triageInfo.taille || '-'} cm</p>
                  </div>
                </div>
                {report.triageInfo.motifVisite && (
                  <div className="mt-3 md:mt-4 flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                    ...
                    <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.triageInfo.motifVisite}</span>
                  </div>
                )}
              </div>
            </MobileSection>
          ...
          {report.labResults && report.labResults.length > 0 && (
            <MobileSection title="Resultats de Laboratoire" icon={FlaskConical} sectionKey="lab">
              <div className="p-3 md:p-4 overflow-x-auto">
                <div className="min-w-[500px] md:min-w-0">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-emerald-100">
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-left text-[10px] md:text-xs font-semibold text-emerald-800">Examen</th>
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Resultat</th>
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Reference</th>
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-left text-[10px] md:text-xs font-semibold text-emerald-800">Interpretation</th>
                      </tr>
                    </thead>
                    <tbody>
                      {report.labResults.map((test, index) => (
                        <tr key={index} className={index % 2 === 1 ? 'bg-gray-50' : ''}>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">
                            <div className="flex items-center gap-1 md:gap-2">
                              {test.isCritical && <AlertTriangle className="w-3 h-3 md:w-4 md:h-4 text-red-500 shrink-0" />}
                              <span className="truncate">{test.testName}</span>
                            </div>
                          </td>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-center font-medium">
                            {test.resultValue} {test.unit}
                          </td>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-[10px] md:text-sm text-center text-gray-600">
                            {test.referenceRange}
                          </td>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">
                            {test.interpretation || '-'}
          ...
          {report.prescription && (
            <MobileSection title="Prescription Medicale" icon={FileText} sectionKey="prescription">
              <div className="p-3 md:p-4">
                {report.prescription.diagnosis && (
                  <div className="mb-3 md:mb-4 flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
                    <span className="text-xs md:text-sm font-semibold text-gray-700">Diagnostic:</span>
                    <span className="text-sm md:text-base border-b border-gray-300 flex-1 pb-1">{report.prescription.diagnosis}</span>
                  </div>
                )}
                <div className="overflow-x-auto">
                  <div className="min-w-[500px] md:min-w-0">
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="bg-emerald-100">
                          <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-left text-[10px] md:text-xs font-semibold text-emerald-800">Medicament</th>
                          <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Posologie</th>
                          <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Duree</th>
                          <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-left text-[10px] md:text-xs font-semibold text-emerald-800">Instructions</th>
                        </tr>
                      </thead>
                      <tbody>
                        {report.prescription.items?.map((item, index) => (
                          <tr key={index} className={index % 2 === 1 ? 'bg-gray-50' : ''}>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">
                              ...
                              <p className="text-[10px] md:text-xs text-gray-500">{item.genericName}</p>
                            </td>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-center">
                              {item.dosage} {item.frequency && <span className="text-gray-500">({item.frequency})</span>}
                            </td>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-center">{item.duration}</td>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">{item.instructions || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
          ...
          {report.pharmacyStatus && (
            <MobileSection title="Dispensation Pharmacie" icon={Pill} sectionKey="pharmacy">
              <div className="p-3 md:p-4 overflow-x-auto">
                <div className="min-w-[400px] md:min-w-0">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="bg-emerald-100">
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-left text-[10px] md:text-xs font-semibold text-emerald-800">Medicament</th>
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Prescrit</th>
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Livre</th>
                        <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Statut</th>
                      </tr>
                    </thead>
                    <tbody>
                      {report.pharmacyStatus.items?.map((item, index) => (
                        <tr key={index} className={index % 2 === 1 ? 'bg-gray-50' : ''}>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">{item.medicationName}</td>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-center">{item.prescribedQty}</td>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-center">{item.deliveredQty}</td>
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-center">
                            <Badge className={`${getPharmacyStatusColor(item.status)} text-[10px] md:text-xs px-1 md:px-2 py-0.5`}>
                              {item.status === 'REFUSE' && <XCircle className="w-2 h-2 md:w-3 md:h-3 mr-1" />}
                              {item.status === 'SERVI' && <CheckCircle2 className="w-2 h-2 md:w-3 md:h-3 mr-1" />}
                              {item.status === 'PARTIEL' && <AlertTriangle className="w-2 h-2 md:w-3 md:h-3 mr-1" />}
                              <span className="hidden sm:inline">{item.status}</span>
                              <span className="sm:hidden">{item.status.substring(0, 3)}</span>
                            </Badge>
                          </td>
                        </tr>
                      ))}
          ...
          {report.billingSummary && (
            <MobileSection title="Facturation" icon={Wallet} sectionKey="billing">
              <div className="p-3 md:p-4">
                {/* Tableau des transactions detaillees - Scrollable sur mobile */}
                <div className="overflow-x-auto mb-4 md:mb-6">
                  <div className="min-w-[600px] md:min-w-0">
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="bg-emerald-100">
                          <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-left text-[10px] md:text-xs font-semibold text-emerald-800">Libelle</th>
                          <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-right text-[10px] md:text-xs font-semibold text-emerald-800">Montant Du</th>
                          <th className="border border-emerald-300 px-2 md:px-3 py-1 md:py-2 text-center text-[10px] md:text-xs font-semibold text-emerald-800">Statut</th>
                        </tr>
                      </thead>
                      <tbody>
                        ...
                        {(report.billingSummary.fraisFiche > 0 || report.isNewPatient) && (
                          <tr className="bg-gray-50">
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">
                              <span className="font-medium">Frais de Fiche</span>
                              {report.isNewPatient && <span className="text-[10px] md:text-xs text-amber-600 ml-1 md:ml-2">(1ere visite)</span>}
                            </td>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-right font-medium">
                              {(report.billingSummary.fraisFiche || 5000).toLocaleString()} $
                            </td>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-center">
                              {report.billingSummary.fraisFichePaid ? (
                                <span className="inline-flex items-center px-1 md:px-2 py-0.5 bg-emerald-100 text-emerald-800 text-[10px] md:text-xs rounded-full">
                                  <CheckCircle2 className="w-2 h-2 md:w-3 md:h-3 mr-1" /> <span className="hidden sm:inline">Paye</span>
                                </span>
                              ) : (
                                <span className="inline-flex items-center px-1 md:px-2 py-0.5 bg-amber-100 text-amber-800 text-[10px] md:text-xs rounded-full">
                                  <AlertCircle className="w-2 h-2 md:w-3 md:h-3 mr-1" /> <span className="hidden sm:inline">En attente</span>
                                </span>
                              )}
                            </td>
                          </tr>
                        ...
                        <tr>
                          ...
                          <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-center">
                            {report.billingSummary.consultationPaid ? (
                              ...
                              <span className="inline-flex items-center px-1 md:px-2 py-0.5 bg-gray-100 text-gray-600 text-[10px] md:text-xs rounded-full">Gratuit</span>
                            )}
                          </td>
                        </tr>
                        ...
                        {report.billingSummary.labDetails && report.billingSummary.labDetails.length > 0 ? (
                          report.billingSummary.labDetails.map((lab, index) => (
                            <tr key={index} className={index % 2 === 1 ? 'bg-gray-50' : ''}>
                              <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">
                                ...
                                <span className="text-[10px] md:text-xs text-gray-500 block">{lab.category}</span>
                              </td>
                              <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-right font-medium">
                          ...
                          <tr className="bg-gray-50">
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">
                              ...
                              <span className="text-[10px] md:text-xs text-gray-500 block">({report.labResults.length} examen{report.labResults.length > 1 ? 's' : ''})</span>
                            </td>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-right font-medium">
                            ...
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-center">
                              {report.billingSummary.labPaid ? (
                                ...
                                <span className="inline-flex items-center px-1 md:px-2 py-0.5 bg-gray-100 text-gray-600 text-[10px] md:text-xs rounded-full">-</span>
                              )}
                            </td>
                          </tr>
                        )}
                          ...
                          <tr>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm">
                              ...
                              {report.pharmacyStatus?.items && (
                                <span className="text-[10px] md:text-xs text-gray-500 block">({report.pharmacyStatus.items.length} produit{report.pharmacyStatus.items.length > 1 ? 's' : ''})</span>
                              )}
                            </td>
                            <td className="border border-gray-300 px-2 md:px-3 py-1 md:py-2 text-xs md:text-sm text-right font-medium">
                              {report.billingSummary.pharmacyAmount?.toLocaleString()} $
                ...
                {/* Recapitulatif des montants - Stack sur mobile */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  {/* Colonne gauche - Totaux */}
                  <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 md:p-4">
                    <h3 className="text-xs md:text-sm font-bold text-emerald-800 mb-2 md:mb-3 uppercase">Recapitulatif</h3>
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs md:text-sm">
                        <span className="text-gray-600">TOTAL GENERAL</span>
                        <span className="font-bold text-base md:text-lg text-emerald-700">{report.billingSummary.totalAmount?.toLocaleString()} $</span>
                      </div>
                      <div className="h-px bg-emerald-200 my-1 md:my-2"></div>
                      <div className="flex justify-between text-xs md:text-sm">
                      ...
                      <div className="flex justify-between text-xs md:text-sm bg-white p-2 rounded border border-emerald-200">
                  ...
                  <div className="flex flex-col justify-center items-center bg-gray-50 border border-gray-200 rounded-lg p-3 md:p-4">
                    <span className="text-xs md:text-sm text-gray-500 mb-2">Statut de paiement</span>
                    <Badge className={`${getPaymentStatusColor(report.billingSummary.paymentStatus)} px-4 md:px-6 py-2 md:py-3 text-sm md:text-base font-bold`}>
                      {report.billingSummary.paymentStatus === 'SOLDE' && <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5 mr-1 md:mr-2" />}
                      {report.billingSummary.paymentStatus === 'PARTIEL' && <AlertCircle className="w-4 h-4 md:w-5 md:h-5 mr-1 md:mr-2" />}
                      {report.billingSummary.paymentStatus === 'NON_PAYE' && <XCircle className="w-4 h-4 md:w-5 md:h-5 mr-1 md:mr-2" />}
      ...
      <style>{`
        @media print {
          ...
          /* Toujours afficher les sections repliables a l'impression */
          .hidden.md\\:block {
