...
public class LivreCaisseService {
    ...
    public LivreCaisseDTO.SyntheseResponse getSynthese(LocalDate dateDebut, LocalDate dateFin) {
        ...
        log.info("📊 [SYNTHESE DEBUG] Jours: {}, Totaux CDF: Entrées={}, Sorties={}, Solde={}",
                journal.size(), totalEntreesCDF, totalSortiesCDF, soldeCumulatifCDF);
        for (LivreCaisseDTO.SyntheseJournaliere jour : journal) {
            log.info("   📅 {}: Entrée={}, Sortie={}, Solde={}",
                    jour.getDate(), jour.getEntreeCDF(), jour.getSortieCDF(), jour.getSoldeCDF());
        }
