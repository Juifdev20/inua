...
const AuthWrapper = ({ children }) => {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      ...
      // ★ Vérifier si l'utilisateur veut forcer l'affichage du login (ex: après reset-password)
      ...
      if (authData && authData.user) {
        ...
        if (biometricEnabled && !isAuthPage && !isPublicPage && !isResetPasswordPage) {
          // Afficher le prompt biométrique avant d'accéder à l'app
          setBiometricUser(authData.user.username);
          setShowBiometric(true);
        }

        // ★ NE PAS rediriger si force=true est présent dans l'URL
        // ★ NE PAS rediriger si on est sur la page reset-password (même si connecté)
        // Redirection automatique depuis les pages publiques/auth (mais pas reset-password)
        if ((isAuthPage || isPublicPage) && !isResetPasswordPage && !forceLogin) {
