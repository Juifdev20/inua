...
const PharmacyHeader = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (parsed.logoUrl) {
        setPharmacyLogo(parsed.logoUrl);
      }
    }
  }, []);
  ...
  // Remplacez par ton hook pharmacie si tu en as un
  ...
  // ═══════════════════════════════════
  //  RECHERCHE MÉDICAMENTS / PATIENTS
  // ═══════════════════════════════════
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (searchQuery.length >= 2) {
        ...
        try {
          ...
          // Recherche médicaments ET patients
          const res = await Promise.all([
            axios.get(`${BACKEND_URL}/api/pharmacy/medications/search?query=${encodeURIComponent(searchQuery)}`, {
              headers: { Authorization: `Bearer ${token}` }
            }),
            axios.get(`${BACKEND_URL}/api/patients/search?query=${encodeURIComponent(searchQuery)}`, {
              headers: { Authorization: `Bearer ${token}` }
            })
          ]);
          
          const medications = res[0].data || [];
          const patients = res[1].data || [];
          
          const combinedResults = [
            ...medications.map(m => ({ ...m, type: 'MEDICATION', icon: 'Pill' })),
            ...patients.map(p => ({ ...p, type: 'PATIENT', icon: 'User' }))
          ];
  ...
  // ICÔNES SPÉCIQUES PHARMACIE
  const getIcon = (type) => {
    switch (type) {
      case 'PRESCRIPTION': return <Package className="w-4 h-4 text-emerald-500" />;
      case 'STOCK_ALERT': return <AlertCircle className="w-4 h-4 text-rose-500" />;
      case 'VENTE': return <DollarSign className="w-4 h-4 text-blue-500" />;
      case 'COMMANDE': return <Receipt className="w-4 h-4 text-purple-500" />;
  ...
  const displayName = `${user?.firstName || user?.prenom || ''} ${user?.lastName || user?.nom || ''}`.trim() || 'Pharmacien';
          ...
          <div className="relative flex-1 hidden sm:block" ref={searchRef}>
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('header.searchPlaceholder') || 'Rechercher médicament, patient, commande...'}
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (item.type === 'PATIENT') {
                            navigate(`/pharmacy/patients/${item.id}`);
                          } else {
                            navigate(`/pharmacy/medications/${item.id}`);
                          }
                          setShowSearchResults(false);
                          setSearchQuery('');
                        }}
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-bold text-foreground">
                            {item.type === 'PATIENT'
                              ? `${item.firstName || item.prenom} ${item.lastName || item.nom}`
                              : item.name || 'Médicament'
                            }
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {item.type === 'PATIENT' 
                              ? item.patientCode || `Dossier #${item.id}`
                              : `Stock: ${item.quantity || 0} | ${item.priceVente || 0}$`
                            }
                          </p>
                        </div>
                        {item.type === 'PATIENT' ? (
                          ...
                          <Pill className="w-4 h-4 ml-auto text-muted-foreground/50" />
                        )}
                      </button>
                    ))
                  ) : (
                ...
                {pharmacyLogo ? (
                  <img
                    ...
                    onError={(e) => {
                      ...
                      setPharmacyLogo(null); // Fall back to default avatar
                    }}
                  />
                ) : (
                  <img
            ...
            {showProfile && (
              <div className="absolute right-0 top-full mt-3 w-64 bg-card rounded-2xl shadow-xl border border-border overflow-hidden py-2 animate-in slide-in-from-top-2 z-50">
                <div className="px-4 py-3 border-b border-border mb-1">
                  <p className="text-xs text-muted-foreground">Session Pharmacie</p>
                  <p className="text-sm font-bold truncate text-foreground">
                    {user?.email || 'pharmacy@hospital.com'}
                  </p>
                </div>
                <Link
