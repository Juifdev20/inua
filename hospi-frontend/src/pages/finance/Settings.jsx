...
export const Settings = () => {
  ...
  const getAvatarUrl = (cacheBust = false) => {
    ...
    if (user.photoUrl.startsWith('data:')) return user.photoUrl;
    if (user.photoUrl.startsWith('http')) return user.photoUrl + ts;
    if (user.photoUrl.startsWith('/uploads')) return `${BACKEND_URL}${user.photoUrl}${ts}`;
    // Handle both profiles and avatars directories
    return `${BACKEND_URL}/uploads/${user.photoUrl}${ts}`;
  };
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      if (response.data?.success) {
        // La réponse contient l'utilisateur mis à jour dans data.data
        const updatedUser = response.data.data;
        updateUser({ photoUrl: updatedUser.photoUrl });
        setAvatarPreview(null);
        showToast('Photo de profil mise à jour');
      } else {
        showToast(response.data?.message || "Erreur lors de l'upload", 'error');
      }
    } catch (err) {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      // Utiliser l'endpoint générique /users/me pour tous les rôles
      const response = await authAPI.api.put('/users/me', {
      ...
      showToast(err.response?.data?.message || 'Erreur mise à jour profil', 'error');
    } finally {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      // Utiliser l'endpoint générique /users/me/password pour tous les rôles
      const response = await authAPI.api.put('/users/me/password', {
      ...
      if (response.data?.success) {
        ...
        setPasswordError(response.data?.message || 'Erreur');
      }
    } catch (error) {
      const msg = error.response?.data?.message;
      if (error.response?.status === 400 || error.response?.status === 401) {
        setPasswordError(msg || "L'ancien mot de passe est incorrect");
      } else {
        setPasswordError(msg || 'Erreur serveur');
      }
    } finally {
  ...
  const handlePreferenceChange = async (key, value) => {
    ...
    try {
      ...
      const response = await authAPI.api.put('/users/me', updateData);
      if (response.data?.success) {
      ...
      showToast(err.response?.data?.message || 'Erreur sauvegarde préférence', 'error');
    } finally {
      ...
      <div className="flex items-center gap-6">
        <div className="relative group">
          <div className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-border bg-muted transition-all group-hover:border-emerald-500/30">
            {(() => {
              const avatarUrl = getAvatarUrl(true);
              return avatarUrl ? (
                <img
