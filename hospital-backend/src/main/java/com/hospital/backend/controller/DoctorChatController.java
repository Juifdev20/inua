...
public class DoctorChatController {
    ...
    /**
     * âœ… Liste des patients (Contacts)
     */
    @GetMapping("/my-patients")
    @PreAuthorize("hasAnyRole('DOCTEUR', 'DOCTOR')") // âœ… Supporte les deux variantes
    ...
    /**
     * âœ… RÃ©cupÃ©rer la conversation
     */
    ...
    /**
     * âœ… Envoyer un message (RÃ©ponse au patient)
     */
    @PostMapping("/send")
    @PreAuthorize("hasAnyRole('DOCTEUR', 'DOCTOR')")
    public ResponseEntity<?> replyToPatient(@RequestBody Map<String, Object> payload, Authentication authentication) {
        try {
            ...
            // âœ… SÃ©curisation de l'ID (Accepte "patientUserId" ou "receiverId")
