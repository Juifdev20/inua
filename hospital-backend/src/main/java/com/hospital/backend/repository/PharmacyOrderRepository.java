...
public interface PharmacyOrderRepository extends JpaRepository<PharmacyOrder, Long> {
    ...
    @Query("SELECT o FROM PharmacyOrder o WHERE (o.archived = false OR o.archived IS NULL) AND o.status IN ('PAYEE', 'LIVREE', 'PARTIELLEMENT_PAYEE', 'PARTIELLEMENT_LIVREE', 'PRETE') AND o.createdAt >= :startDate AND o.createdAt <= :endDate ORDER BY o.createdAt DESC")
    ...
    /**
     * ═════════════════════════════════════════════════════════════════
     * EXPIRATION - Commandes expirées
     * ═════════════════════════════════════════════════════════════════
     * 
     * Récupère les commandes en attente de paiement créées avant une date donnée.
     * Utilisé par le service d'expiration pour annuler les commandes non payées.
     * 
     * @param status Statut de la commande (EN_ATTENTE_PAIEMENT)
     * @param createdAt Date limite de création
     * @return Liste des commandes expirées
     */
    List<PharmacyOrder> findByStatusAndCreatedAtBefore(PharmacyOrderStatus status, LocalDateTime createdAt);
}
