 -- Script pour peupler la table consultation_services avec les services existants
-- Basé sur les examens prescrits déjà existants dans la base de données
...
WHERE pe.consultation_id NOT IN (
    SELECT DISTINCT cs.consultation_id 
    FROM consultation_services cs
    WHERE cs.service_id = pe.service_id
);
...
SELECT 
    ...
    SUM(s.prix) as calculated_total,
    GROUP_CONCAT(s.nom) as services_names
