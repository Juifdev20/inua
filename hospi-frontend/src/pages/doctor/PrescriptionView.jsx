...
const PrescriptionView = ({ 
  ...
  const [newMedication, setNewMedication] = useState({
    ...
    dosage: '',
    frequency: '',
    timeSlots: [], // Horaires spécifiques (matin, midi, soir)
    duration: '',
    quantityPerDose: '',
    instructions: ''
  });

  // Options de fréquence avec horaires prédéfinis
  const frequencyOptions = [
    { value: '1', label: '1 fois/jour', times: ['08:00'] },
    { value: '2', label: '2 fois/jour', times: ['08:00', '20:00'] },
    { value: '3', label: '3 fois/jour', times: ['08:00', '12:00', '20:00'] },
    { value: '4', label: '4 fois/jour', times: ['08:00', '12:00', '16:00', '20:00'] },
  ];

  const timeSlotLabels = {
    '08:00': { label: 'Matin', icon: '🌅' },
    '12:00': { label: 'Midi', icon: '☀️' },
    '16:00': { label: 'Après-midi', icon: '🌤️' },
    '20:00': { label: 'Soir', icon: '🌙' },
  };
  ...
  const addMedication = () => {
    ...
    if (!newMedication.frequency) {
      toast.error('Veuillez sélectionner la fréquence de prise');
      return;
    }

    const frequencyLabel = frequencyOptions.find(opt => opt.value === newMedication.frequency)?.label || '';
    const medicationToAdd = {
      ...
      frequencyDisplay: frequencyLabel
    };
  ...
  const handleValidateAndClose = async () => {
    ...
    try {
      ...
      const items = medications.map(med => ({
        ...
        timeSlots: med.timeSlots || [], // Horaires pour notifications
                    ...
                    {medications.map((med) => (
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {med.dosage} • {med.frequencyDisplay || `${med.frequency} fois/jour`} • {med.duration}
                          </p>
                          {med.timeSlots && med.timeSlots.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-1">
                              {med.timeSlots.map((time) => (
                                ...
                                >
                                  {timeSlotLabels[time]?.icon} {timeSlotLabels[time]?.label} ({time})
                                </span>
                              ))}
                            </div>
                          )}
                ...
                <div className="border-t border-border/60 pt-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      {/* Affichage des horaires sélectionnables */}
                      {newMedication.frequency && (
                        <div className="mt-2 p-2 bg-muted/30 rounded-lg">
                          <p className="text-[10px] text-muted-foreground mb-2">Horaires de prise :</p>
                          <div className="flex flex-wrap gap-2">
                            {frequencyOptions
                              .find(opt => opt.value === newMedication.frequency)?.times
                              .map((time) => (
                                <div
                                  key={time}
                                  className={`flex items-center gap-1 px-2 py-1 rounded-md text-xs ${
                                    newMedication.timeSlots.includes(time)
                                      ? 'bg-emerald-100 text-emerald-700 border border-emerald-300'
                                      : 'bg-gray-100 text-gray-500'
                                  }`}
                                >
                                  <span>{timeSlotLabels[time]?.icon}</span>
                                  <span>{timeSlotLabels[time]?.label}</span>
                                  <span className="text-[10px] opacity-70">({time})</span>
                                </div>
                              ))}
                          </div>
                          <p className="text-[10px] text-muted-foreground mt-2 italic">
                            💡 Les notifications seront envoyées 15 min avant chaque horaire
                          </p>
                        </div>
                      )}
                    </div>
