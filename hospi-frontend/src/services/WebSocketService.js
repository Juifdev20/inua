...
class WebSocketService {
  ...
  /**
   * 💰 Souscription aux notifications de paiement confirmé pour la Pharmacie
   * @param {Function} onMessageCallback - Callback appelé quand un paiement est confirmé
   */
  subscribeToPharmacyPayments(onMessageCallback) {
    if (!this.client || !this.connected) {
      console.warn('WebSocket non connecte - notifications pharmacie indisponibles');
      return null;
    }

    try {
      const subscription = this.client.subscribe('/topic/pharmacy/paiements-confirme', (message) => {
        try {
          const notification = JSON.parse(message.body);
          console.log('💰💊 [WEBSOCKET] Paiement confirmé reçu:', notification);
          if (onMessageCallback) {
            onMessageCallback(notification);
          }
        } catch (error) {
          console.error('Error parsing pharmacy notification:', error);
        }
      });

      this.subscriptions.push(subscription);
      console.log('✅ [WEBSOCKET] Abonné aux notifications pharmacie');
      return subscription;
    } catch (error) {
      console.error('Erreur subscription pharmacie:', error);
      return null;
    }
  }
