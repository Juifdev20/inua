...
const ServiceManager = () => {
  ...
  const loadServices = async () => {
    try {
      ...
      // Gérer le format {success, content, total} du backend
      const servicesList = Array.isArray(data) ? data : (data?.content || []);
      setServices(servicesList);
    } catch (error) {
      console.error(error);
      toast.error(t('common.error') || 'Erreur chargement services');
    } finally {
      setLoading(false);
    }
  };
