...
export const Admissions = () => {
  ...
  // ✅ État pour l'annulation d'admission
  ...
  const calculatePricing = async (patientId, serviceId) => {
    ...
    try {
      const response = await pricingService.calculateAdmissionPrice(patientId, serviceId);
      // Extraire les données de la réponse (response.content contient AdmissionPricingDTO)
      const pricingData = response.content || response;
  ...
  // ═══════════════════════════════════════
  const openCancelModal = (admission) => {
    if (admission.status !== 'EN_ATTENTE' && admission.status !== 'ARRIVED' && admission.status !== 'WAITING_PAYMENT') {
      toast.error("Seules les admissions en attente de paiement peuvent être annulées");
      return;
    }
    setAdmissionToCancel(admission);
    setCancelReason('');
    setCancelModalOpen(true);
  };

  const handleCancelAdmission = async () => {
    if (!admissionToCancel) return;

    setIsCancelling(true);
    try {
      await admissionService.cancelAdmission(admissionToCancel.id, cancelReason || null);

      toast.success(
        <div className="flex flex-col gap-1">
          <span className="font-bold">Admission annulée</span>
          <span className="text-xs opacity-80">
            {admissionToCancel.patientName} a été retiré(e) de la file d'attente
          </span>
        </div>
      );
      ...
      setCancelReason('');
      fetchDashboardData(); // Rafraîchir la liste
    } catch (error) {
      console.error('Erreur annulation:', error);
      const errorMsg = error?.message || error?.error || "Erreur lors de l'annulation";
      toast.error(
        <div className="flex flex-col gap-1">
          ...
          <table className="w-full">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-xs font-bold text-muted-foreground uppercase">Actions</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={adm.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      {(adm.status === 'EN_ATTENTE' || adm.status === 'ARRIVED' || adm.status === 'WAITING_PAYMENT') && (
                        <button
      ...
      <Dialog open={cancelModalOpen} onOpenChange={setCancelModalOpen}>
        <DialogPortal>
          ...
          <DialogContent className={cn(
            ...
            "bg-background border border-border rounded-2xl p-0 shadow-2xl"
          )}>
            ...
            <div className="p-4 space-y-4">
              <div className="p-3 bg-amber-500/10 rounded-xl">
                <p className="text-sm text-amber-600">
                  <span className="font-bold">⚠️ Attention :</span> Cette action est irréversible. Le patient sera retiré de la file d'attente.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Ex: Patient souhaite reporter le rendez-vous..."
                  rows={3}
                  className="w-full p-3 border border-border rounded-xl bg-background text-foreground focus:border-red-500 focus:outline-none resize-none text-sm"
                />
              </div>
            </div>
