...
const CaisseLaboratoire = () => {
  ...
  // ✅ Calcul des totaux par devise (multi-devises)
  const adjustedTotalsByCurrency = useMemo(() => {
    return adjustedExams.reduce((acc, exam) => {
      if (!exam.active) return acc;
      const currency = exam.currency || 'USD';
      const amount = exam.unitPrice * exam.quantity;
      acc[currency] = (acc[currency] || 0) + amount;
      return acc;
    }, {});
  }, [adjustedExams]);

  const originalTotalsByCurrency = useMemo(() => {
    return adjustedExams.reduce((acc, exam) => {
      const currency = exam.currency || 'USD';
      acc[currency] = (acc[currency] || 0) + exam.unitPrice;
      return acc;
    }, {});
  }, [adjustedExams]);

  // ✅ Totaux combinés (pour compatibilité)
  const adjustedTotal = useMemo(() => {
    return Object.values(adjustedTotalsByCurrency).reduce((sum, val) => sum + val, 0);
  }, [adjustedTotalsByCurrency]);
  ...
  // ✅ Stats par devise (multi-devises)
  const stats = useMemo(() => {
    ...
    const unpaidByCurrency = unpaid.reduce((acc, t) => {
    ...
    const paidByCurrency = paid.reduce((acc, t) => {
      const currency = t.currency || 'USD';
    ...
    return {
      ...
      unpaidByCurrency,
      paidByCurrency,
      // Pour compatibilité - total en USD (ou première devise)
      unpaidAmount: unpaidByCurrency.USD || Object.values(unpaidByCurrency)[0] || 0,
      paidAmount: paidByCurrency.USD || Object.values(paidByCurrency)[0] || 0,
    };
  }, [labTests]);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 p-5 text-white shadow-lg shadow-amber-500/20 group hover:shadow-xl hover:shadow-amber-500/30 transition-all duration-300">
            ...
            <div className="relative z-10">
              ...
              <p className="text-lg font-bold mt-1">
                {Object.entries(stats.unpaidByCurrency).map(([curr, amt], i) => (
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 p-5 text-white shadow-lg shadow-emerald-500/20 group hover:shadow-xl hover:shadow-emerald-500/30 transition-all duration-300">
            ...
            <div className="relative z-10">
              ...
              <p className="text-lg font-bold mt-1">
                {Object.entries(stats.paidByCurrency).map(([curr, amt], i) => (
                  <span key={curr}>{i > 0 && ' + '}{formatCurrency(amt, curr)}</span>
                ))}
              </p>
                    ...
                    >
                      ...
                      <div className="text-right shrink-0">
                        <p className={cn('text-lg font-black', paid ? 'text-emerald-500' : 'text-foreground')}>
                          {formatCurrency(test.examTotalAmount || test.totalAmount, test.currency)}
                        </p>
                        <p className="text-[10px] text-muted-foreground">{test.currency || 'USD'}</p>
                        <div className={cn(
            ...
            <Card className="border-none shadow-lg rounded-2xl overflow-hidden animate-in slide-in-from-right-4 duration-300">
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div>
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center pt-2 border-t border-border/30">
                          ...
                          <div className="text-right">
                            <span className="text-xs text-muted-foreground">{formatCurrency(exam.unitPrice, exam.currency)} x {exam.quantity}</span>
                            <p className="text-lg font-black text-foreground">
                              {formatCurrency(exam.unitPrice * exam.quantity, exam.currency)}
                              <span className="text-xs text-muted-foreground ml-1">{exam.currency || 'USD'}</span>
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  {/* Totaux adaptés au dark mode - MULTI-DEVISE */}
                  <div className="mt-5 p-5 rounded-2xl bg-muted/40 border border-border/50 shadow-sm space-y-3">
                    {/* Affichage par devise */}
                    {Object.entries(adjustedTotalsByCurrency).map(([currency, amount]) => (
                      <div key={currency} className="flex justify-between items-center">
                        <span className="text-sm font-bold text-foreground flex items-center gap-2">
                          <DollarSign className="w-5 h-5" />
                          Total {currency}
                        </span>
                        <span className="text-xl font-black text-emerald-500">{formatCurrency(amount, currency)}</span>
                      </div>
                    ))}
                    ...
                    {Object.keys(originalTotalsByCurrency).length > 0 && 
                     ...
                     Object.entries(originalTotalsByCurrency).some(([c, val]) => 
                       val > (adjustedTotalsByCurrency[c] || 0)
                     ) && (
