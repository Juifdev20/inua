                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="bg-gradient-to-br from-muted/50 to-muted/30 rounded-xl p-4 space-y-3 border border-border/50">
                    ...
                    {(selectedAdmission.items || selectedAdmission.services || []).length > 0 ? (
                      ...
                      <>
                        {/* Afficher les frais de dossier (registration fee) */}
                        {(selectedAdmission.registrationFee > 0) && (
                          <div className="flex justify-between items-center text-sm py-1">
                            <span className="text-muted-foreground">Frais de dossier</span>
                            <span className="font-bold">{formatCurrency(selectedAdmission.registrationFee)}</span>
                          </div>
                        )}
                        {/* Afficher les frais de service/consultation */}
                        {(selectedAdmission.serviceFee > 0) && (
                          <div className="flex justify-between items-center text-sm py-1">
                            <span className="text-muted-foreground">Consultation médicale</span>
                            <span className="font-bold">{formatCurrency(selectedAdmission.serviceFee)}</span>
                          </div>
                        )}
                        {/* Fallback pour les anciennes admissions sans les nouveaux champs */}
                        {(!selectedAdmission.registrationFee && !selectedAdmission.serviceFee && selectedAdmission.totalAmount > 0) && (
                          <div className="flex justify-between items-center text-sm py-1">
