...
// ★ IMPORTS POUR LA CRÉATION AUTOMATIQUE DE COMPTE
...
const AddPatient = () => {
  ...
  // ★ ÉTATS POUR LA MODALE DE SUCCÈS AVEC IDENTIFIANTS
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Appel API pour créer le patient
      ...
      const response = await api.post('/v1/patients', data, {
        headers: { 
          'Content-Type': 'multipart/form-data' 
        }
      });
      
      console.log('🆕 Réponse création patient:', response.data);
      
      // ★ RÉCUPÉRER L'ID DU PATIENT CRÉÉ
      // La réponse a la structure: { success: true, data: { id: ..., ... }, ... }
      const patientId = response.data?.data?.id || response.data?.id || response.data?.patient?.id;
      console.log('🆕 ID patient extrait:', patientId);
      ...
      if (patientId) {
        ...
        try {
          const accountResponse = await accountCreationApi.createPatientAccount(patientId);
          
          if (accountResponse.success) {
            // Afficher la modale avec les credentials générés
            setGeneratedCredentials({
              username: accountResponse.username,
              generatedPassword: accountResponse.generatedPassword,
              patient: accountResponse.patient
            });
            
            // ★ ENVOI D'EMAIL SI L'EMAIL EST PRÉSENT
            const patientEmail = response.data?.data?.email || data.get('email') || '';
            
            if (patientEmail && patientEmail.trim() !== '') {
              try {
                await accountCreationApi.sendCredentialsEmail({
                  to: patientEmail,
                  firstName: accountResponse.patient?.firstName || '',
                  lastName: accountResponse.patient?.lastName || '',
                  username: accountResponse.username,
                  password: accountResponse.generatedPassword,
                  userType: 'PATIENT'
                });
                
                toast.success("Patient, compte créés et email envoyé !", { 
                  description: `Identifiant: ${accountResponse.username} | Email envoyé à ${patientEmail}`,
                  icon: <CheckCircle2 className="h-5 w-5 text-[#37f49e]" />
                });
              } catch (emailError) {
                console.error("⚠️ Erreur envoi email:", emailError);
                // L'email a échoué mais on continue avec la modale
                toast.success("Patient et compte créés avec succès !", { 
                  description: `Identifiant: ${accountResponse.username} (email non envoyé)`,
                  icon: <CheckCircle2 className="h-5 w-5 text-[#37f49e]" />
                });
              }
            } else {
              // Pas d'email → afficher uniquement la modale
              toast.success("Patient et compte créés avec succès !", { 
                description: `Identifiant: ${accountResponse.username} | Modale affichée (pas d'email)`,
                icon: <CheckCircle2 className="h-5 w-5 text-[#37f49e]" />
              });
            }
          ...
          const errorMsg = accountError.response?.data?.error || accountError.response?.data?.message || "Erreur inconnue";
          console.error("❌ Message d'erreur backend:", errorMsg);
          
          // Le patient est créé mais le compte a échoué - on affiche quand même le succès partiel
          toast.success("Patient créé", { 
            description: `Le dossier a été créé, mais la création du compte a échoué: ${errorMsg}`,
            icon: <CheckCircle2 className="h-5 w-5 text-[#37f49e]" />
          });
