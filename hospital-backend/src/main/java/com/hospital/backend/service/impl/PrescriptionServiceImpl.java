...
public class PrescriptionServiceImpl implements PrescriptionService {
        ...
        if (dto.getItems() != null && !dto.getItems().isEmpty()) {
            for (PrescriptionDTO.PrescriptionItemDTO itemDTO : dto.getItems()) {
                ...
                PrescriptionItem item = PrescriptionItem.builder()
                    ...
                    .timeSlots(itemDTO.getTimeSlots())
    ...
    public PrescriptionDTO createPrescription(PrescriptionDTO dto) {
        ...
        if (dto.getItems() != null && !dto.getItems().isEmpty()) {
            ...
            for (int i = 0; i < dto.getItems().size(); i++) {
                ...
                // ✅ LOG: Vérifier la devise du médicament
                log.info("💱 [PRESCRIPTION] Médicament: {} - SaleCurrency: {}, UnitPrice: {}",
                    medication.getName(), medication.getSaleCurrency(), medication.getUnitPrice());
    ...
    @Override
    @Transactional(readOnly = true)
    public PageResponse<PrescriptionDTO> getPaidPrescriptions(LocalDateTime startDate, LocalDateTime endDate, Pageable pageable) {
        log.info("💰 Récupération des prescriptions payées du {} au {}", startDate, endDate);
        
        Page<Prescription> paidPrescriptions = prescriptionRepository.findPaidPrescriptionsByDateRange(startDate, endDate, pageable);
        
        List<PrescriptionDTO> dtos = paidPrescriptions.getContent().stream()
            .map(this::mapToDTO)
            .collect(Collectors.toList());
        
        PageResponse<PrescriptionDTO> response = new PageResponse<>();
        response.setContent(dtos);
        response.setPage(paidPrescriptions.getNumber());
        response.setSize(paidPrescriptions.getSize());
        response.setTotalElements(paidPrescriptions.getTotalElements());
        response.setTotalPages(paidPrescriptions.getTotalPages());
        response.setFirst(paidPrescriptions.isFirst());
        response.setLast(paidPrescriptions.isLast());
        return response;
    }
