...
const FinanceHeader = () => {
  ...
  // 💰 Compteurs de badges (mêmes que la sidebar)
  ...
  // ═══════════════════════════════════
  //  COMPTAGE DES NOTIFICATIONS FINANCE
  // ═══════════════════════════════════
  const loadPendingCounts = useCallback(async () => {
    try {
      ...
      // Pharmacie (File - nouvelle)
      let pharmaQueueCount = 0;
      try {
        const pharmaRes = await getPendingPaymentsForFinance(0, 1);
        pharmaQueueCount = pharmaRes.data?.totalElements || 0;
      } catch (e) {}
      ...
      const total = admissionsCount + labCount + pharmaQueueCount + expensesCount;

      setPendingCounts({
        admissions: admissionsCount,
        laboratory: labCount,
        pharmacy: 0, // Ancien système
        pharmacyQueue: pharmaQueueCount,
        expenses: expensesCount,
        total
      });
    } catch (error) {
      console.log('🔔 [HEADER] Erreur chargement compteurs');
    }
  }, []);
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-3 w-80 bg-card rounded-2xl shadow-xl border border-border overflow-hidden ring-1 ring-black/5 animate-in slide-in-from-top-2 z-50">
                ...
                <div className="max-h-80 overflow-y-auto">
                  ...
                  {/* Notifications système (si pas de pending) */}
                  {notifications.length > 0 && pendingCounts.total === 0 && (
                  ...
                  {pendingCounts.total === 0 && notifications.length === 0 && (
                    <div className="p-8 text-center text-muted-foreground text-sm">
                      ...
                      <p>À jour ! Aucun élément en attente</p>
                    </div>
                  )}
                </div>
                <div
