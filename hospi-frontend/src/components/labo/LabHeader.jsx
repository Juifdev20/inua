...
import { useLaboratory } from '../../context/LaboratoryContext'; // à créer si besoin
...
const FlagGB = ({ className }) => (
  <svg viewBox="0 0 640 480" className={className}>
    ...
    <path fill="#C8102E" d="m424 281 216 159v40L369 281h55zm-184 20 6 35L54 480H0l240-179zM640 0v3L391 191l2-44L590 0h50zM0 0l239 176h-60L0 42V0h75z" />
...
const LabHeader = () => {
  ...
  const { toggleMobileSidebar } = useLaboratory(); // si tu as un context labo
      ...
      if (searchQuery.length >= 2) {
        ...
        try {
          ...
          const res = await axios.get(
            `${BACKEND_URL}/api/laboratory/search?query=${encodeURIComponent(searchQuery)}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
          setSearchResults(res.data || []);
          setShowSearchResults(true);
        } catch (err) {
          console.error("Erreur recherche labo:", err);
          setSearchResults([]);
        } finally {
  ...
  useEffect(() => {
    ...
    stompClient.connect({}, () => {
      // Topic personnel de l'utilisateur
      stompClient.subscribe(`/topic/laboratory/${user.id}`, (message) => {
        const newNotif = JSON.parse(message.body);
        handleNewNotification(newNotif);
      });

      // Topic général du laboratoire (tous les techniciens reçoivent)
      stompClient.subscribe('/topic/laboratory/general', (message) => {
        const newNotif = JSON.parse(message.body);
        handleNewNotification(newNotif);
      });

      // Topic global des notifications (tous les utilisateurs)
  ...
  // Gestion d'une nouvelle notification
  const handleNewNotification = (newNotif) => {
    setNotifications((prev) => {
      // Anti-doublon
      const isDuplicate = prev.some(n => 
        n.id === newNotif.id || 
        (n.message === newNotif.message && Math.abs(new Date(n.timestamp) - new Date(newNotif.timestamp)) < 5000)
      );
      if (isDuplicate) return prev;
      return [{ ...newNotif, read: false }, ...prev];
    });
    ...
    // Notification navigateur (desktop)
    if (user.notificationEnabled !== false && 'Notification' in window && Notification.permission === 'granted') {
      new Notification(newNotif.title || 'Nouvelle alerte labo', {
        body: newNotif.message || 'Une nouvelle action est disponible au laboratoire.',
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'RESULT_READY': return <Check className="w-4 h-4 text-emerald-500" />;
      case 'SAMPLE_MISSING': return <AlertTriangle className="w-4 h-4 text-amber-500" />;
      case 'VALIDATION': return <ShieldCheck className="w-4 h-4 text-blue-500" />;
  ...
  const getAvatarUrl = useCallback(() => {
    ...
    if (!user?.photoUrl || user.photoUrl === '')
      return `https://ui-avatars.com/api/?name=${firstName}+${lastName}&background=6366f1&color=fff`;
  ...
  }, [user?.photoUrl, user?.firstName, user?.lastName, user?.prenom, user?.nom]);

  const displayName = `${user?.firstName || user?.prenom || ''} ${user?.lastName || user?.nom || ''}`.trim() || 'Technicien Labo';
          ...
          <div className="relative flex-1 hidden sm:block" ref={searchRef}>
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('header.searchPlaceholder') || 'Rechercher patient, test ou consultation...'}
                  ...
                  {searchResults.length > 0 ? (
                    searchResults.map((p) => (
                      ...
                      >
                        ...
                        <ClipboardList className="w-4 h-4 ml-auto text-muted-foreground/50" />
                      </button>
                    ))
                  ) : (
              ...
              <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-emerald-500/20 bg-muted flex items-center justify-center shadow-sm">
                <img
                  ...
                  onError={(e) => {
                    ...
                    const fn = user?.firstName || user?.prenom || 'L';
                    const ln = user?.lastName || user?.nom || 'B';
                    e.target.src = `https://ui-avatars.com/api/?name=${fn}+${ln}&background=6366f1&color=fff`;
                  }}
                />
              </div>
            ...
            {showProfile && (
              <div className="absolute right-0 top-full mt-3 w-64 bg-card rounded-2xl shadow-xl border border-border overflow-hidden py-2 animate-in slide-in-from-top-2 z-50">
                <div className="px-4 py-3 border-b border-border mb-1">
                  <p className="text-xs text-muted-foreground">Session Laboratoire</p>
                  <p className="text-sm font-bold truncate text-foreground">
                    {user?.email || 'labo@hospital.com'}
                  </p>
                </div>
                <Link
                  to="/labo/settings"
