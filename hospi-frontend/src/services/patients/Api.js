...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      ...
      // Ces endpoints retournent 401 si le mot de passe est incorrect (normal)
      ...
      console.warn('🔒 [API] Session expirée ou non authentifiée (401)');
      // Nettoyer le stockage local
      localStorage.removeItem('token');
      localStorage.removeItem('user');
      // Rediriger vers la page de login si pas déjà sur /login
